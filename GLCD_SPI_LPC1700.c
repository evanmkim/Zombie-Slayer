...
unsigned short getPixelCol(uint32_t sy,uint32_t sx, const bool map[24][32]){
	//printf("px, sx: [%d,%d]\n", myPlayer->x, sx);
	if (map[sy/10][sx/10])
		return Black;
	else
...
void GLCD_DrawMap (const bool map[24][32]) {
  uint32_t x, y, h, w;
	unsigned short test;
	x = 0;
	y= 0 ;
  ...
  for (int i = 0; i <= (h-1)*w; i += w) {
    for (int j = 0; j < w; j++) {
			//printf("i/w, j: [%d,%d]\n", i/w, j);
			//printf("px, sx: [%d]\n", myPlayer->x);
			test = getPixelCol(((uint32_t)i)/w, (uint32_t)j, map);
			//printf("%04X\n", test);
      wr_dat_only (test);
    }
  }
...
void GLCD_DrawSprite(struct sprite* s){
	uint16_t i, j, x, y, dir;
	//osMutexAcquire(s->mutex_id, 0U);
	x = s->x;
	y = s->y;
	dir = s->dir;	
	//osMutexRelease(s->mutex_id);
	
	if((s->oldx-x) != 0 || (s->oldy-y) != 0){
		GLCD_SetWindow (s->oldx, s->oldy, s->size, s->size);
		wr_cmd(0x22);
		wr_dat_start();
	...
	//GLCD_Bitmap(x, y, s->size, s->size, s->bitmap);
  unsigned short *bitmap_ptr = (unsigned short *)s->bitmap;

  GLCD_SetWindow (x, y, s->size, s->size);
  ...
  for (i = 0; i < s->size; i++) {
    for (j = 0; j < s->size; j++) {
			if(dir == RIGHT)
				wr_dat_only (bitmap_ptr[i*s->size+j]);
    	if(dir == LEFT)
				wr_dat_only (bitmap_ptr[i*s->size+(s->size-1-j)]);
    	if(dir == UP)
				wr_dat_only (bitmap_ptr[(s->size-1-i)+j*s->size]);
    	if(dir == DOWN)
				wr_dat_only (bitmap_ptr[i+j*s->size]);
		}
  }
  wr_dat_stop();
...
void GLCD_DeleteSprite(struct sprite* s){
	GLCD_SetWindow (s->oldx, s->oldy, s->size, s->size);
	wr_cmd(0x22);
	wr_dat_start();
	//printf("sadsds");
	
	for (uint16_t i = 0; i < (s->size*s->size); i++){
		//printf("%d\n", i);
		wr_dat_only (White);
	}
	wr_dat_stop();
